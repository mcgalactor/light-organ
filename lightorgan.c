...
bool doubletricker[MY_NUM_PINS]={false,false,false,false,false,false};
...
void midi_process(snd_seq_event_t *ev)
{
	...
	//printf("lastplayed ");
       ...
       if( isPlayChannel(ev->data.note.channel) ) {
		...
		if (lastplayed[pinIdx]==0 || (currenttimestamp-lastplayed[pinIdx]>60000)||doubletricker[pinIdx]){
			digitalWrite(pinIdx, 1); 
			doubletricker[pinIdx]=false;
		}else{
			digitalWrite(pinIdx, 0);
			doubletricker[pinIdx]=true;
			//printf("double note\n");
		}
